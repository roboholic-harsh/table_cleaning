...
class AutomaticSorter(Node):
    def __init__(self):
        ...
        self.bin_green = {'x': -0.5, 'y': 0.0, 'z': 0.6}  # Back
    ...
    def execute_mission(self, point, color_type):
        ...
        # 3. GRAB
        print(">>> GRASPING...")
        # Use negative position trick for tight grip if using JTC
        ...
        # 5. DEFINE PATH TO BIN (Robust Logic)
        # We create a list of targets. If Green, we add the intermediate stop.
        bin_targets = []
        
        if color_type == "RED":
            bin_targets.append(self.bin_red)
        elif color_type == "BLUE":
            bin_targets.append(self.bin_blue)
        elif color_type == "GREEN":
            # GREEN PATH: Blue Bin (Intermediate) -> Green Bin (Final)
            bin_targets.append(self.bin_blue)
            bin_targets.append(self.bin_green)

        # 6. EXECUTE BIN MOVES
        print(f"Moving to {color_type} Bin (Steps: {len(bin_targets)})...")
        
        for i, target in enumerate(bin_targets):
            current_pose.position.x = target['x']
            current_pose.position.y = target['y']
            current_pose.position.z = target['z']
            
            print(f"  -> Moving to Waypoint {i+1}/{len(bin_targets)}...")
            
            # CRITICAL FIX: If this move fails, STOP. Do not drop the block.
            if not self.move_to_pose(current_pose):
                print(f"!!! MOVE FAILED at Waypoint {i+1} !!! Aborting Drop.")
                # Optional: Try to return Home to reset
                self.move_to_home() 
                self.control_gripper("OPEN")
                self.is_busy = False
                return
        ...
        # 7. DROP (Only reached if ALL moves succeeded)
        print(f"*** REACHED FINAL BIN ({color_type}) ***")
        self.control_gripper("OPEN")
        time.sleep(1.0) 
        
        # 8. RETURN PATH (For Green, go back via Blue)
        if color_type == "GREEN":
             print("Returning via Intermediate (Blue)...")
             current_pose.position.x = self.bin_blue['x']
             current_pose.position.y = self.bin_blue['y']
             current_pose.position.z = self.bin_blue['z']
