...
class ColouredTFDetector(Node):
    ...
    def image_callback(self, msg):
        ...
        mask_r1 = cv2.inRange(hsv, np.array([0, 100, 220]), np.array([10, 255, 255]))
        mask_r2 = cv2.inRange(hsv, np.array([170, 100, 220]), np.array([180, 255, 255]))
        mask_red = mask_r1 + mask_r2
        self.process_color(cv_image, mask_red, (0, 0, 20), self.red_pub, "R")

        # 2. GREEN
        mask_green = cv2.inRange(hsv, np.array([35, 100, 220]), np.array([85, 255, 255]))
        self.process_color(cv_image, mask_green, (0, 20, 0), self.green_pub, "G")

        # 3. BLUE
        mask_blue = cv2.inRange(hsv, np.array([100, 100, 220]), np.array([140, 255, 255]))
        self.process_color(cv_image, mask_blue, (20, 0, 0), self.blue_pub, "B")
    ...
    def process_color(self, img_to_draw, mask, viz_color_bgr, publisher, label_prefix):
        ...
        for obj in detected_objects:
            ...
            if obj == target_obj:
                ...
                # Thin Box
                cv2.rectangle(img_to_draw, (x, y), (x + w, y + h), viz_color_bgr, 2)
